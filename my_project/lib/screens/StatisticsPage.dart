...
final currentDate =
    DateTime.now().subtract(Duration(days: 1)); // once assigned can be changed
          ...
          actions: [
            IconButton(
              ...
              onPressed: () async {
                ...
                if (await sp.getBool('week_sim') == true ||
                    await sp.getBool('week_sim') == null) {
                  ...
                  Map weekDist =
                      await _getWeekDistance(startWeek, oneWeekLater);
                  List distance = await weekDist.values.toList();

                  Map weekTime =
                      await _getWeekActivityTime(startWeek, oneWeekLater);
                  ...
                  for (int i = 0; i < 7; i++) {
                    ...
                    await Provider.of<DatabaseRepository>(context,
                            listen: false)
                        .insertAnswers(Questionnaire(1, days[i], 0, 0, 0, 0));
                    await Provider.of<DatabaseRepository>(context,
                            listen: false)
                        .insertAchievements(
                            Achievements(1, days[i], today_LoS));

                    await sp.setBool('week_sim',
                        false); // a check to be sure that the simulation can be done one time only
                  }
                }
